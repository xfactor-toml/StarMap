...
export async function MintPlasma (account : string, amount : number) : Promise<number> {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            const contract = new web3.eth.Contract(jsonABIs.ERC20ABI, network.contracts.plasma)
...
export async function ApprovePlasma (owner: account, amount: number, spender : account = network.contracts.starNFT) : Promise<number> {
    return new Promise(async (resolve, reject) => {
    ...
    try {
        ...
        const w3 = new web3.eth.Contract(jsonABIs.ERC20ABI, network.contracts.plasma)
