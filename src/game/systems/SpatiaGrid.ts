...
export const SpatialGrid = (() => {
    ...
    class SpatialHashSlow {
        ...
        _GetCellIndex(position) {
            const x = MyMath.sat((position[0] - this._bounds[0][0]) / (
                this._bounds[1][0] - this._bounds[0][0]));
            const y = MyMath.sat((position[1] - this._bounds[0][1]) / (
    ...
    return {
        SpatialHashCrap: SpatialHashCrap,
        SpatialHashSlow: SpatialHashSlow
    };
